...
getElementPtrExpr:
	OpGetelementptr inBounds? '(' type ',' typeConst (
		',' gepIndex
	)* ')';
...
getElementPtrInst:
	OpGetelementptr inBounds? type ',' typeValue (',' typeValue)* (
		',' metadataAttachment
	)*;
truncInst:
...
vaargInst: OpVaArg typeValue ',' type (',' metadataAttachment)*;
landingPadInst:
...
operandsField:
	KwOperandsLabel '{' (mdField (',' mdField)*)? '}';
diTemplateValueParameterField:
	tagField
	| nameField
	| typeField
...
debugInfoForProfilingField:
	KwDebugInfoForProfilingLabel boolConst;
...
stringLengthExpressionField:
	KwStringLengthExpressionLabel mdField;
stringLocationExpressionField:
